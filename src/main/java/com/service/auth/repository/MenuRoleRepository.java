...
public interface MenuRoleRepository extends JpaRepository<MenuRole, Long> {
    ...
    @Query("SELECT p FROM MenuRole p WHERE p.id.userRole = :user_role AND menu.parentId IS NULL AND menu.lang = :lang ORDER BY menu.order ASC")
	List<MenuRole> findByUserRoleNoParent(@Param("user_role") String userrole, @Param("lang") String lang);

    @Query("SELECT p FROM MenuRole p WHERE p.id.userRole = :user_role AND menu.parentId = :parentId AND menu.lang = :lang ORDER BY menu.order ASC")
    ...
    @Modifying
    @Transactional
    @Query("DELETE FROM MenuRole u WHERE u.id.userRole = :role")
